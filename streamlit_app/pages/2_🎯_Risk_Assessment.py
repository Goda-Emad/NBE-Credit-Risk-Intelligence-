...
T = {
    'ar': {
        ...
        'rec_med':  '⚠️ التوصية: مراجعة يدوية مطلوبة. يُنصح بطلب تحقق إضافي من الدخل أو تخفيض مبلغ القرض بنسبة 20%.',
    ...
    'en': {
        ...
        'rec_med':  '⚠️ Recommendation: Manual review required. Consider requesting additional income verification or reducing loan amount by 20%.',
...
st.markdown(
    ...
    '  --bg:#001f15; --card:rgba(255,255,255,0.04);'
    '  --border:rgba(201,168,76,0.25); --gray:#8a9bb0;'
    '}'
    ...
    '[data-testid="stMetricLabel"]{color:var(--gray)!important;font-size:12px!important;'
    ...
    /* BUTTON */
    '.stButton>button{'
    '  background:linear-gradient(135deg,var(--gold),#a07a1e)!important;'
    ...
    '  transition:all 0.3s!important;letter-spacing:0.5px;}'
    ...
    '@keyframes pulseResult{0%,100%{box-shadow:0 0 20px rgba(201,168,76,0.2)}'
...
st.markdown(
    f'<div style="background:linear-gradient(135deg,rgba(0,18,8,0.9),rgba(0,42,29,0.9));'
...
if submitted:
    ...
    with c2:
        ...
        st.markdown(
            ...
            f'<span style="color:#4ade80;font-weight:800;font-family:JetBrains Mono,monospace;">'
            f'{proba[1]*100:.1f}%</span></div>'
            f'<div style="height:10px;background:rgba(255,255,255,0.06);border-radius:5px;overflow:hidden;">'
            f'<div style="height:10px;width:{proba[1]*100:.1f}%;background:linear-gradient(90deg,#15803d,#4ade80);'
            f'border-radius:5px;"></div></div></div>'
            f'<div>'
            f'<div style="display:flex;justify-content:space-between;margin-bottom:6px;">'
            f'<span style="color:#f87171;font-size:13px;font-weight:700;">❌ {t["bad_prob"]}</span>'
            f'<span style="color:#f87171;font-weight:800;font-family:JetBrains Mono,monospace;">'
            f'{proba[0]*100:.1f}%</span></div>'
            f'<div style="height:10px;background:rgba(255,255,255,0.06);border-radius:5px;overflow:hidden;">'
            f'<div style="height:10px;width:{proba[0]*100:.1f}%;background:linear-gradient(90deg,#991b1b,#f87171);'
            f'border-radius:5px;"></div></div></div></div>',
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        ...
        f'💡 {"التوصية" if is_ar else "Recommendation"}</div>'
...
st.markdown(
    ...
    f'font-size:13px;font-weight:600;transition:all 0.3s;"'
